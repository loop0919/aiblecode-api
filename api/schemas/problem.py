...
class Category(BaseModel):
    ...
    description: str = Field(..., example="これはサンプルカテゴリです。", description="Category Description")
...
class CategoryCreateResponse(BaseModel):
    ...
    message: str = Field(..., example="Category created successfully", description="Message")
...
class Problem(BaseModel):
    ...
    statement: str = Field(..., example="正整数 $N$ が与えられます。 $N$ を $2$ 倍した値を出力してください。", description="Problem Statement")
...
class ProblemCreate(BaseModel):
    ...
    title: str = Field(..., example="サンプル問題", description="Problem Title")
    statement: str = Field(..., example="正整数 $N$ が与えられます。 $N$ を $2$ 倍した値を出力してください。", description="Problem Statement")
    category_path_id: str = Field(..., example="99_category", description="Category Path ID")
    time_limit: float = Field(..., example=2.0, description="Time Limit")
    memory_limit: int = Field(..., example=256, description="Memory Limit")
...
class ProblemCreateResponse(BaseModel):
    status: Literal["success", "failed"] = Field(..., example="success", description="Status")
    message: str = Field(..., example="Problem created successfully", description="Message")
...
class TestcaseCreateResponse(BaseModel):
    status: Literal["success", "failed"] = Field(..., example="success", description="Status")
    message: str = Field(..., example="Testcase created successfully", description="Message")
